...
describe('favorite blog', () => {
  ...
  test('when list has only one blog, equals that blog', () => {
    ...
    const favoriteBlog = listWithOneBlog.filter(blog => blog.likes === result)
    expect(favoriteBlog).toEqual(listWithOneBlog)
  }) 

  test('of a bigger list is calculated right', () => {
    const result = listHelper.favoriteBlog(listWithMultipleBlogs)
    const favoriteBlog = listWithMultipleBlogs.filter(blog => blog.likes === result)
    expect(favoriteBlog).toEqual(listWithOneBlog)
  }) 
})
...
describe('most blogs', () => {
  ...
  test('when list has only one blog, equals that blog', () => {
    ...
    expect(mostBlogs).toEqual({author: 'Edsger W. Dijkstra', blogs: 1})
  })
